...
$(function(){
	...
	$('#table_practice_teaching').datagrid({
		url:'/BusinessFiles/getPracticeTeachinglist',
		queryParams: {
			termDate: term
		},
		rownumbers:true,
		...
		columns:[[
			...
			{title:'专/兼职',field:'positiontype',width:100,align:'center'},
			{title:'实践课程名称',field:'courseName',width:100,align:'center'},
			{title:'实践课程类型',field:'courseType',width:100,sortable:true,align:'center'},
			{title:'班级代码',field:'tclass',width:100,align:'center',sortable:true},
			{title:'实践周数',field:'weekNum',width:100,align:'center',sortable:true},
...
function searchRecord(){
	...
	$("#table_practice_teaching").datagrid('load',{
		termDate: term,
		cond: $('#cond').val()
	});
}
...
function deleteRecord(){
	var rows = $('#table_practice_teaching').datagrid('getChecked');
	if (rows.length <= 0) {
		alert("请选择要删除的记录");
		return;
	}
	...
	$.messager.confirm("提示", "您确定要删除这些数据吗？", function (res) {
		if (res) {	
			$.ajax({
				url : '/BusinessFiles/deletePracticeTeaching',
				type : "post",
				...
				success : function(data) {
					...
					for(var i=rows.length-1; i>=0; i--){
						var index = $('#table_practice_teaching').datagrid('getRowIndex',rows[i]);
						$('#table_practice_teaching').datagrid('deleteRow',index);
					}
						$.messager.show({
							title:'提示',
							timeout:3000,
...
function saveRecord(){
	...
	if(Id){
		$.ajax({
			url : "/BusinessFiles/updatePracticeTeaching",
			type : "post",
			contentType : false,
			processData : false,
			data : data,
			success : function(data) {
				if(data.msg){
					$('#table_practice_teaching').datagrid('reload');
					closeDlg();
					$.messager.show({
						title:'提示',
						timeout:3000,
	...
	$.ajax({
		url : "/BusinessFiles/addPracticeTeaching",
		type : "post",
		contentType : false,
		processData : false,
		data : data,
 ...
 function exportData(){
 	var rows = $('#table_practice_teaching').datagrid('getSelections');
 	var cond="";
	for(var i=0;i<rows.length;i++){
		cond+=("ids="+rows[i].id+"&");
	}
	cond=cond.substring(0,cond.length-1);
	 window.open("/BusinessFiles/exportPracticeTeaching?"+cond); 
 }
 function ImportData(){
	...
	$.ajax({
		url : "/BusinessFiles/importPracticeTeaching",
		type : "post",
		contentType : false,
		processData : false,
		data : data,
	...
	function showEditDlg(flag){
		...
		changeTermEdit();
		if(flag){
			var row= $('#table_practice_teaching').datagrid('getSelected');
...
<body class="easyui-layout">
	...
	<div data-options="region:'center',title:'实践教学',collapsible:false,border:false" style="overflow:hidden;">
		<table id="table_practice_teaching">
		...
		<div id="dlg_import" closed="true" class="easyui-dialog" title="导入理论授课任务" style="width:300px;padding:10px">
			<form id="form_import" method="post" enctype="multipart/form-data">
				...
				<div align="center">
					...
					<a  class="easyui-linkbutton" data-options="iconCls:'icon-down'" href="template/PracticeTeaching.xls">下载模板</a>				
				</div>	     
			</form>
		</div>
 
	   <div id="dlg_edit" closed="true" class="easyui-dialog" title="实践教学" style="width:300px;height:auto;padding:5px;font-size:14px; font-family:Tahoma,Verdana,微软雅黑,新宋体"  data-options="iconCls: 'icon-save',closable:false">
	   		<form  id="form_edit" method="post"  > 
				<table cellpadding="3" >
					...
					<tr>
						<td>课程类型:</td>
						<td ><input class="easyui-combobox" data-options="valueField: 'name',textField: 'name',editable:false,panelHeight:'auto',	url:'jsondata/PracticeTeaching_courseType.json'" style="width:110px;" type="text" id="courseTypeTSPrT" name="courseType"  /></td>
					</tr>
					<tr>
						<td>学生人数:</td>
					...
					<tr>
						<td>实践周数:</td>
						<td ><input class="easyui-numberbox" data-options="precision:1,min:0" style="width:135px;" type="text" id="weekNum" name="weekNum"  /></td>
					</tr>
					<tr>
						<td>折算学时</td>
