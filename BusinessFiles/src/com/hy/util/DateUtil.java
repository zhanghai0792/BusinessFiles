...
public class DateUtil {
	public static Date date = new Date();//今日日期
	...
	public static Date YMDHMSToDateTime(String date){
		...
		public static String getFirstDayInMonth(){
			Date  day = new Date();
	        String   firstdate = (day.getYear()+1900)+ '-' +(day.getMonth()+1)+ "-01"; 
	        return firstdate;  
	     }
		//得到当前月的最后一天
		@SuppressWarnings("deprecation")
		public static String getLastDayInMonth(){ 
			...
			//int month=(today.getMonth()+1)<10?"0"+(today.getMonth()+1):today.getMonth()+1;
			int month=today.getMonth()+1;
			Date  day = new Date(year,month,0);
	        String lastdate = year + "-" + (month<10?("0"+(today.getMonth()+1)):(today.getMonth()+1))+ "-" + day.getDate();//获取当月最后一天日期    
	        return lastdate;  
	     }
		...
		public static String getBeforeDate(int i){
			if(i<0)return "";
					...
					SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");
					 ...
					 long fro = date.getTime();
					 long to = fro-i * 24 * 60 * 60 * 1000;
					 Date m= new Date(to);
					return sdf.format(m);
			}
		}
		//得到当前日期之前之后i天的日期对象 i>0,今天之后i天,i<0,今天之前i天
